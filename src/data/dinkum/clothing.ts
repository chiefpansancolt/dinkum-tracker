import { Clothing, ClothingSlot } from "@/types/dinkum";
...
export const getClothingByType = (type: string): Clothing[] => {
  return clothing.filter((item) => item.type === type);
};

export const getClothingBySlot = (slot: ClothingSlot): Clothing[] => {
  return clothing.filter((item) => item.slot.includes(slot));
};

export const getClothingBySet = (set: string): Clothing[] => {
  return clothing.filter((item) => item.set === set);
};
