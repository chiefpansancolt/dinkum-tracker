...
const LicensesTab = forwardRef<LicensesTabHandle, LicensesTabProps>(
	({ licenses: licensesState }, ref) => {
		const params = useParams();
		const playthroughId = typeof params.id === "string" ? params.id : "";
		const [searchQuery, setSearchQuery] = useState("");
		const [categoryFilter, setCategoryFilter] = useState("all");
		const [localLicensesState, setLocalLicensesState] = useState(licensesState);

		const isDirty = useRef(false);

		useEffect(() => {
			setLocalLicensesState(licensesState);
		}, [licensesState]);

		const isPreviousLevelObtained = (licenseId: string, level: number) => {
			if (level === 1) return true; // First level has no prerequisites
		...
		const handleToggleLicenseLevel = (licenseId: string, level: number) => {
			...
			// If trying to check this level, ensure all previous levels are checked
			if (!currentValue && !isPreviousLevelObtained(licenseId, level)) {
				return; // Do nothing if prerequisites aren't met
			}

			// If unchecking this level, also uncheck all higher levels
			if (currentValue) {
				const updates: Record<string, boolean> = {
					[licenseKey]: false,
				};

				// Find this license object
		...
		const toggleAllLevelsForLicense = (license: License, setToValue: boolean) => {
			...
			if (setToValue) {
				// Set all levels to true in sequence
