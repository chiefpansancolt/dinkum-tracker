...
import { SignWritingRecipesTabHandle, SignWritingRecipesTabProps } from "@/types/dinkum";

const SignWritingRecipesTab = forwardRef<SignWritingRecipesTabHandle, SignWritingRecipesTabProps>(
	({ unlocked }, ref) => {
		...
		const saveSignWritingRecipes = () => {
			if (!playthroughId || !isDirty.current) return false;

			const success = updatePlaythroughData(playthroughId, {
				signWritingRecipes: localRecipesState,
			});
			...
			<div className="space-y-6">
				<div>
					<div className="mb-2 flex items-center justify-between">
						<h1 className="text-primary text-2xl font-bold">
							Sign Writing Recipes ({getunlockedCount()} / {signWritingRecipes.length}
							)
						</h1>
						<Badge color="blue" size="lg">
							{Math.round((getunlockedCount() / signWritingRecipes.length) * 100)}%
							unlocked
						</Badge>
					</div>

					{isDirty.current && (
						<SaveAlert message="Your sign writing recipe collection has not been saved yet." />
					)}
				</div>
				...
				<div className="mb-4">
					<p className="text-primary font-medium">
						Showing {filteredRecipes.length} of {signWritingRecipes.length} recipes •
...
SignWritingRecipesTab.displayName = "SignWritingRecipesTab";

export default SignWritingRecipesTab;
